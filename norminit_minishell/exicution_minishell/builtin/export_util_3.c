...
/*   Updated: 2025/07/18 22:49:09 by maskour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_exist_env(t_env *env, char *key, char *value)
{
	...
	while (arg.current)
	{
		if (!strncmp(arg.current->data_env, key, strlen(key)) && \
			(arg.current->data_env[strlen(key)] == '=' || \
			arg.current->data_env[strlen(key)] == '\0'))
		{
