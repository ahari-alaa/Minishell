...
/*   Updated: 2025/07/18 21:14:51 by maskour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_or_add_env(t_env **env, char *key, char *value)
{
	...
	while (current)
	{
		if (!ft_strncmp(current->data_env, key, ft_strlen(key)) && \
		current->data_env[ft_strlen(key)] == '=')
		{
			tmp = ft_strjoin(key, "=");
			if (!tmp)
				return ;
			...
			return (current->data_env = add_equal, (void)0);
		}
