...
static t_file *create_file_entry(t_token *current)
{
	t_file	*file;
	char	*tmp;
...
static int process_redirection(t_cmd *cmd, t_token *current)
{
	t_file	*file;
	...
	file = create_file_entry(current);
	if (!file)
		return (0);
...
static int parse_redirections(t_cmd *cmd, t_token **tokens)
{
	...
	while (current && current->type != TOKEN_PIPE)
	{
		...
		{
			if (!process_redirection(cmd, current))
				return (0);
			current = current->next;
		}
		current = current->next;
