...
static void	wait_for_children(pid_t last_pid, t_shell *shell_ctx)
{
	...
	(1) && (wstatus = 0, saw_sigint = 0, wpid = wait(&wstatus));
	while (wpid > 0)
	{
	...
	char **env, t_env *env_list, t_exec_ctx *ctx)
{
	...
	{
		handle_parent_process(ctx->i, ctx->prev_pipe, ctx->pipes,
			ctx->cmd_count);
		*(ctx->last_pid) = pid;
	}
	else
		return (handle_pipeline_error(cmds, ctx->cmd_count,
				"minishell: fork", ctx->shell_ctx));
	return (0);
}
	...
	t_env *env_list, t_exec_ctx *exec_ctx)
{
	...
	if (!cmds || exec_ctx->cmd_count <= 0)
		return ;
	if (!process_all_heredocs(cmds, exec_ctx->cmd_count,
			env, exec_ctx->shell_ctx))
		return ;
