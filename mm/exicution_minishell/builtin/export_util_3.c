...
/*   Updated: 2025/07/19 12:50:30 by maskour          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	update_exist_env(t_env *env, char *key, char *value)
{
	...
	while (arg.current)
	{
		if (!ft_strncmp(arg.current->data_env, key, ft_strlen(key)) && \
			(arg.current->data_env[ft_strlen(key)] == '=' || \
			arg.current->data_env[ft_strlen(key)] == '\0'))
		{
			free(arg.current->data_env);
			if (value)
			{
				arg.tmp = ft_strjoin(key, "=");
				if (!arg.tmp)
					return (1);
				arg.current->data_env = ft_strjoin(arg.tmp, value);
				free(arg.tmp);
			}
			else
				arg.current->data_env = strdup(key);
			return (1);
		}
